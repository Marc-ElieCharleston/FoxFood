...
export function generateOrderRecapPDF({
  ...
  const dishRows = dishes.map((dish, idx) => [
    idx + 1,
    dish.name,
    dish.variantName && dish.variantName !== 'Classique' ? dish.variantName : '-'
  ])
  ...
  if (ingredients && Object.keys(ingredients).length > 0) {
    ...
    categoryOrder.forEach(category => {
      if (ingredients[category] && ingredients[category].length > 0) {
        // Ajouter le nom de catégorie comme ligne de groupe
        ingredientRows.push([{ content: categoryLabels[category] || category, colSpan: 2, styles: { fontStyle: 'bold', fillColor: [240, 240, 240] } }])
...
export function generateAdminWeeklyRecapPDF({
  ...
  categoryOrder.forEach(category => {
    if (ingredients[category] && ingredients[category].length > 0) {
      ...
      doc.text(categoryLabels[category] || category, 15, yPos)
      yPos += 6
