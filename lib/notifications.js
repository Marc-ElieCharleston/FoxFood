...
/**
 * Récupérer les overrides de plats d'un utilisateur (masquage, renommage, substitutions par plat)
 */
async function getUserDishOverrides(userId) {
  try {
    const result = await sql`
      SELECT dish_id, custom_name, remove_ingredients, substitute_ingredients
      FROM user_dish_overrides
      WHERE user_id = ${userId} AND action = 'modify'
    `
    const overrides = new Map()
    result.rows.forEach(r => overrides.set(r.dish_id, r))
    return overrides
  } catch (error) {
    console.error('Erreur récupération dish overrides:', error)
    return new Map()
  }
}
...
export async function getShoppingListData({
  ...
  try {
    ...
    const dishOverrides = userId ? await getUserDishOverrides(userId) : new Map()
    ...
    // Pré-charger les ingrédients de substitution des overrides par plat
    const allSubToIds = []
    ...
    if (allSubToIds.length > 0) {
      const subIngResult = await sql`
        SELECT id, name, category FROM ingredients WHERE id = ANY(${allSubToIds})
      `
    ...
    for (const row of ingredientsResult.rows) {
      const dishOverride = dishOverrides.get(row.dish_id)
      ...
      if (dishOverride) {
        ...
        if (removeIds.includes(row.id)) {
          console.log(`   ✗ Ingrédient retiré (override plat): "${row.name}" du plat "${row.dish_name}"`)
          continue
        }
      }
      ...
      let dishName = dishOverride?.custom_name || row.dish_name
      ...
      if (dishOverride) {
        ...
        const sub = subs.find(s => s.from_ingredient_id === row.id)
        if (sub) {
          const repl = dishSubReplacements.get(sub.to_ingredient_id)
          if (repl) {
            console.log(`   ✓ Substitution plat: "${row.name}" → "${repl.name}" (plat "${row.dish_name}")`)
      ...
      // 3. Appliquer les remplacements globaux (si pas déjà substitué par un override plat)
      if (!substituted && userReplacements.has(row.id)) {
        const replacement = userReplacements.get(row.id)
        console.log(`   ✓ Remplacement global: "${row.name}" (ID:${row.id}) → "${replacement.replacementName}"`)
