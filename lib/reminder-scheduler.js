import { sql } from './db'
import { sendUserReminder, sendWeeklyShoppingReminder, notifyAdminMissingSelection, relancerDemandesEnAttente } from './notifications'
...
export async function processReminders() {
  ...
  // Relance des demandes d'accès laissées sans réponse. Volontairement hors de
  // la boucle ci-dessus : elle ne parcourt que les clients déjà validés, donc
  // une personne en attente n'y apparaît jamais — c'est exactement ce qui les
  // rendait invisibles.
  try {
    const relance = await relancerDemandesEnAttente()
    results.pending_relances = relance.relancees || 0
    if (!relance.success && relance.error) {
      results.errors.push(`Relance demandes d'accès: ${relance.error}`)
    }
  } catch (error) {
    console.error("Erreur relance demandes d'accès:", error)
    results.errors.push(`Relance demandes d'accès: ${error.message}`)
  }
