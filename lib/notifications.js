...
/**
 * Notifier l'admin avec la liste de courses du client
 */
export async function notifyAdminShoppingList({ adminEmail, sendEmail: doEmail, userName, userEmail, shoppingListHtml, householdSize }) {
  const subject = `Liste de courses - ${userName}`

  const emailContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2>🛒 Liste de courses</h2>
      <p><strong>${userName}</strong> (${userEmail})</p>
      <p style="color: #6b7280;">Quantités pour <strong>${householdSize} personne${householdSize > 1 ? 's' : ''}</strong></p>
  ...
  await logNotification({
    notification_type: 'admin_shopping_list',
    recipient_email: adminEmail,
    method: 'email',
    subject,
    content: emailContent,
