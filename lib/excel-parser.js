...
function parseIngredients(ingredientsStr) {
  ...
  for (const part of parts) {
    ...
    const match = part.match(/^(\d+(?:[.,]\d+)?)\s*(g|kg|ml|L|pc|pcs|piece|pieces|c\.?a\.?s\.?|cas)?\s*[:\-]?\s*(.+)$/i)
    ...
    if (match) {
      ...
      if (name) {
        ingredients.push({ quantity: 100, unit: 'g', name })
      }
    }
  }
...
export function generateDishTemplate() {
  const data = [
    ['Nom', 'Catégorie', 'Description', 'Saisons', 'Ingrédients'],
    ['Poulet rôti aux herbes', 'viandes', 'Poulet fermier rôti avec pommes de terre', 'toutes', '200g:poulet, 150g:pommes de terre, 2c.a.s:huile olive, 1pc:oignon, 2pc:gousse ail'],
    ['Saumon grillé légumes', 'poissons', 'Pavé de saumon avec haricots verts', 'printemps, ete', '180g:saumon, 120g:haricots verts, 30g:beurre, 1pc:citron'],
    ['Risotto champignons', 'vegetation', 'Risotto crémeux aux champignons de Paris', 'automne, hiver', '100g:riz arborio, 150g:champignons, 50ml:vin blanc, 30g:parmesan, 1pc:échalote'],
    ['Boeuf bourguignon', 'viandes', 'Mijoté de boeuf au vin rouge', 'automne, hiver', '200g:boeuf, 100g:carottes, 100g:oignons, 100ml:vin rouge, 50g:lardons'],
    ['Curry de légumes', 'vegetation', 'Curry doux aux légumes de saison', 'toutes', '100g:pois chiches, 100g:courgettes, 100g:aubergines, 100ml:lait coco, 1c.a.s:curry']
  ]
