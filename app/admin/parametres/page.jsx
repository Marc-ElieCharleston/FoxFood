...
export default function AdminSettingsPage() {
  ...
  const handleRunMigrations = async () => {
    const confirmed = confirm(
      ...
      'Cette action va mettre à jour la base de données avec les nouvelles fonctionnalités.\n' +
      'Les migrations déjà exécutées seront automatiquement ignorées.'
    )
    ...
    try {
      ...
      toast.info('Exécution des migrations en cours...')
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        toast.error(data.error || 'Erreur lors des migrations')
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto min-h-[calc(100vh-200px)]">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6 space-y-6 max-w-2xl">
        ...
        <div className="border-b pb-6">
          <h2 className="text-lg font-bold mb-4">⚙️ Maintenance de la base de données</h2>
          <p className="text-sm text-gray-600 mb-4">
            Exécuter les migrations pour mettre à jour la base de données avec les nouvelles fonctionnalités.
            Les migrations déjà exécutées seront automatiquement ignorées.
          </p>
          <button
            type="button"
          ...
          >
            {runningMigrations ? '⏳ Exécution...' : '▶️ Exécuter les migrations'}
          </button>
        </div>
