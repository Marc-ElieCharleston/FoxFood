...
/**
 * Notifier l'admin qu'un client a fait sa sélection (avec liste de courses)
 */
export async function notifyAdminOnSelection({ adminEmail, adminPhone, sendEmail: doEmail, sendSMS: doSMS, userName, userEmail, selectedDishes, shoppingListHtml, householdSize }) {
  const subject = `${userName} a fait sa sélection`

  const dishList = selectedDishes.map(d => `<li>${d}</li>`).join('')

  // Inclure la liste de courses si fournie
  const shoppingSection = shoppingListHtml ? `
      <hr style="margin: 30px 0; border: none; border-top: 1px solid #eee;" />
      <h3 style="color: #ea580c;">🛒 Liste de courses</h3>
  ...
  try {
    ...
    for (const [category, ingredients] of byCategory) {
      ...
      for (const ing of ingredients) {
        ...
        if (category === 'epice') {
          ingredientsHtml += `<li><strong>${ing.name}</strong></li>`
        } else {
...
/**
 * Envoyer un récapitulatif des plats sélectionnés (toutes semaines groupées)
 */
export async function sendUserDishSelectionSummary({
  userId,
  userName,
  userEmail,
  weeklySelections, // { week0: { date, dishes: [dishNames] }, week1: ..., }
  totalDishes
}) {
  if (!userEmail || !weeklySelections || Object.keys(weeklySelections).length === 0) {
    return { success: false, error: 'Données manquantes' }
  }

  const subject = `✅ Vos plats sélectionnés (${totalDishes} plats)`
  ...
  for (const [weekKey, weekData] of Object.entries(weeklySelections)) {
    ...
    const dishList = weekData.dishes.map(d => `<li>${d}</li>`).join('')

    weeksHtml += `
      <div style="margin-bottom: 25px; padding: 15px; background-color: #f9fafb; border-radius: 8px; border-left: 4px solid #ea580c;">
        <h3 style="margin: 0 0 10px 0; color: #374151; font-size: 16px;">📅 Semaine du ${formattedDate}</h3>
        <ul style="margin: 0; padding-left: 20px; line-height: 1.8;">
          ${dishList}
        </ul>
      </div>
  ...
  const emailContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2>Bonjour ${userName} ! 👋</h2>
      <p>Votre sélection a bien été enregistrée. Voici un récapitulatif de vos <strong>${totalDishes} plats</strong> :</p>
  ...
  await logNotification({
    notification_type: 'dish_selection_summary',
    recipient_user_id: userId,
    recipient_email: userEmail,
    method: 'email',
    subject,
...
export async function sendUserShoppingListGrouped({
  ...
  try {
    ...
    for (const dishId of allDishIds) {
      const variantId = allVariants[dishId]
      if (variantId) {
        variantIds.push(variantId)
      } else {
        const defaultVariant = await sql`
