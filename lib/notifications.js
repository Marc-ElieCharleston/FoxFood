import { sql } from './db'
import { loadDishOverrides, loadOverrideIngredients, applyDishOverride } from './dish-overrides'
...
export async function notifyAdminOnSelection({ adminEmail, adminPhone, sendEmail: doEmail, sendSMS: doSMS, userName, userEmail, weeklyData, householdSize }) {
  ...
  for (const [weekKey, weekInfo] of Object.entries(weeklyData)) {
    ...
    if (weekInfo.shoppingListHtml) {
      shoppingHtml = `
        <div style="background-color: #fff7ed; padding: 15px; border-radius: 8px; margin-top: 15px;">
          <h4 style="margin: 0 0 10px 0; color: #ea580c; font-size: 14px;">&#127859; Ce qu'il y a dans chaque plat</h4>
...
export async function getShoppingListData({
  ...
  try {
    // Récupérer les remplacements globaux et les overrides par plat
    const userReplacements = userId ? await getUserIngredientReplacements(userId) : new Map()
    const dishOverrides = userId ? await loadDishOverrides(userId) : new Map()
    ...
    if (userId && dishOverrides.size > 0) {
      console.log(`🔧 [User ${userId}] ${dishOverrides.size} override(s) de plat actif(s)`)
    }
    ...
    // ...et, EN PLUS, garder le détail plat par plat pour l'email du chef.
    // Le client fait ses courses : sa liste est groupée par rayon. Le chef
    // cuisine et vérifie plat par plat — un total de 400 g de saumon ne lui dit
    // pas lequel de ses cinq plats en demande combien. Même boucle, mêmes
    // overrides, mêmes remplacements : deux calculs séparés divergeraient.
    const parPlat = new Map()
    for (const [dishId, rows] of rowsByDish) {
      const dishOverride = dishOverrides.get(dishId)
      const dishName = dishOverride?.custom_name || rows[0].dish_name
      ...
      for (const item of applied) {
        ...
        if (item.source === 'substituted') {
          console.log(`   ✓ Substitution plat: "${item.ref.name}" → "${meta.name}" (plat "${item.ref.dish_name}")`)
        } else if (item.source === 'added') {
          console.log(`   ✚ Ingrédient ajouté (override plat): "${meta.name}" au plat "${dishName}"`)
        }
        ...
        if (item.source === 'original' && userReplacements.has(ingredientId)) {
          ...
          console.log(`   ✓ Remplacement global: "${item.ref.name}" (ID:${ingredientId}) → "${replacement.replacementName}"`)
        ...
        const unit = item.unit || (meta ? meta.default_unit : null)

        // Détail du plat : la quantité qu'il pèsera pour ce plat-là.
        if (!parPlat.has(dishId)) parPlat.set(dishId, { name: dishName, ingredients: [] })
        parPlat.get(dishId).ingredients.push({
          name: ingredientName,
        ...
        if (aggregatedIngredients.has(key)) {
          ...
          existing.quantity += item.quantity * householdSize
    ...
    // Le détail par plat voyage ACCROCHÉ à la Map plutôt que dans un objet de
    // retour : changer la forme du retour casserait tous les appelants, qui
    // itèrent directement dessus. Les rayons continuent de sortir à l'identique.
    byCategory.parPlat = [...parPlat.values()]
 ...
 */
/**
 * Les ingrédients SOUS CHAQUE PLAT — le format du chef.
 *
 * « Plat 1 ingrédients / Plat 2 ingrédients / etc. » : c'est avec ça qu'il
 * prépare et qu'il vérifie. La liste par rayon reste celle du client, qui fait
 * ses courses — les deux formats coexistent, ils ne répondent pas à la même
 * question. Quantités déjà multipliées par la taille du foyer.
 */
export function generateIngredientsParPlatHtml(parPlat) {
  if (!parPlat || parPlat.length === 0) return ''

  let html = ''
  for (const plat of parPlat) {
    html += `
      <div style="margin-bottom: 16px; padding-bottom: 12px; border-bottom: 1px solid #e5e7eb;">
        <h4 style="color: #374151; margin: 0 0 8px 0; font-size: 15px;">${plat.name}</h4>
        <ul style="margin: 0; padding-left: 20px;">
    `
    if (plat.ingredients.length === 0) {
      html += `<li style="margin-bottom: 4px; color: #ea580c;">Pas encore de recette pour ce plat.</li>`
    } else {
      for (const ing of plat.ingredients) {
        const qty = ing.quantity > 0
          ? `: ${ing.quantity % 1 === 0 ? ing.quantity : parseFloat(ing.quantity).toFixed(1)} ${ing.unit || ''}`.trimEnd()
          : ''
        html += `<li style="margin-bottom: 4px;">${ing.name}${qty}</li>`
      }
    }
    html += '</ul></div>'
  }
  ...
  for (const cat of categoryOrder) {
    if (grouped.has(cat)) {
      ...
      for (const ing of items) {
        if (ing.quantity > 0 && ing.category !== 'epice') {
          const qty = ing.quantity % 1 === 0 ? ing.quantity : parseFloat(ing.quantity).toFixed(1)
          html += `<li style="margin-bottom: 4px;">${ing.name}: ${qty} ${ing.unit || ''}</li>`
        } else {
          html += `<li style="margin-bottom: 4px;">${ing.name}</li>`
        }
      }
      html += '</ul></div>'
    }
  ...
  try {
    ...
    for (const cat of categoryOrder) {
      ...
      for (const ing of ingredients) {
        const quantityStr = ing.quantity > 0 && ing.category !== 'epice' ? `${Math.round(ing.quantity)} ${ing.unit}` : ''
    ...
    for (const [dishName, ingredients] of ingredientsByDish) {
      const ingredientsList = ingredients.map(i => {
        if (i.quantity > 0 && i.category !== 'epice') {
...
/**
 * Domaines écartés de la liste des admins à notifier.
 *
 * `emeric@foxfood.com` et `dev@foxfood.com` sont deux comptes de seed encore
 * marqués `role = 'admin'`. Le site est foxfood.FR ; foxfood.COM appartient à un
 * tiers (MX GoDaddy actifs, vérifié le 08/09/2026). Les notifier revient à
 * envoyer le nom, l'email et le téléphone d'un nouveau client chez quelqu'un
 * d'autre — et les rejets qui s'ensuivent abîment la réputation d'expédition,
 * ce qui finit par pousser les vrais mails en spam.
 */
const DOMAINES_ADMIN_EXCLUS = ['foxfood.com']

/**
 * Les admins réellement joignables. Renvoie un tableau, pas un résultat SQL.
 */
async function getAdminsANotifier() {
  const admins = await sql`
    SELECT email, name FROM users WHERE role = 'admin' AND approval_status = 'approved'
  `
  return admins.rows.filter(a => {
    const domaine = String(a.email || '').split('@')[1]
    return domaine && !DOMAINES_ADMIN_EXCLUS.includes(domaine.toLowerCase())
  })
}

/**
 * Notifier l'admin (Emeric) qu'un nouveau compte a été créé et attend validation
 */
export async function notifyAdminPendingUser({ pendingUser, approvalToken }) {
  try {
    const admins = await getAdminsANotifier()
    if (admins.length === 0) {
      console.warn('Aucun admin à notifier pour validation utilisateur')
      return { success: false, error: 'Aucun admin' }
    }

    const baseUrl = process.env.NEXTAUTH_URL || 'https://foxfood.fr'
    const approveUrl = `${baseUrl}/api/admin/approve-user?token=${approvalToken}&action=approve`
    const rejectUrl = `${baseUrl}/api/admin/approve-user?token=${approvalToken}&action=reject`

    const subject = `🔔 Nouvelle demande d'accès FoxFood - ${pendingUser.name}`
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #ea580c;">🔔 Nouvelle demande d'accès</h2>
        <p>Une personne souhaite créer un compte sur FoxFood :</p>
        <div style="background: #f9fafb; padding: 16px; border-radius: 8px; margin: 16px 0;">
          <p style="margin: 4px 0;"><strong>Nom :</strong> ${pendingUser.name}</p>
          <p style="margin: 4px 0;"><strong>Email :</strong> ${pendingUser.email}</p>
          ${pendingUser.phone ? `<p style="margin: 4px 0;"><strong>Téléphone :</strong> ${pendingUser.phone}</p>` : ''}
        </div>
        <p>Souhaites-tu lui donner accès à l'application ?</p>
        <div style="margin: 24px 0; text-align: center;">
          <a href="${approveUrl}" style="display: inline-block; background: #16a34a; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold; margin: 0 8px;">✅ Valider l'accès</a>
          <a href="${rejectUrl}" style="display: inline-block; background: #dc2626; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold; margin: 0 8px;">❌ Refuser</a>
        </div>
        <p style="color: #6b7280; font-size: 13px;">Tant que tu n'as pas validé ce compte, cette personne ne pourra pas accéder à l'application.</p>
      </div>
    `
    ...
    await logNotification({
      notification_type: 'pending_user_approval',
      recipient_email: admins.map(a => a.email).join(', '),
      method: 'email',
      subject,
      content: `Demande d'accès: ${pendingUser.name} <${pendingUser.email}>`,
      status: results.every(r => r.success) ? 'sent' : 'failed'
    })

    return { success: true, sent: results.length }
  } catch (error) {
    console.error('Erreur notification admin nouveau compte:', error)
    return { success: false, error: error.message }
  }
}
 ...
 *
 * `notifyAdminPendingUser` ne part qu'une fois, à l'inscription. Un mail manqué
 * — spam, mauvaise journée, téléphone — et la personne attend indéfiniment sans
 * que rien ne le signale : le 08/09/2026, quatre comptes étaient bloqués, le
 * plus ancien depuis deux mois, tous avec leur lien de validation jamais cliqué.
 *
 * Le compteur ne vit pas dans une colonne : chaque relance laisse une ligne
 * `pending_user_reminder` dans `notifications_log`, dont le contenu porte
 * l'email entre chevrons. Compter ces lignes suffit, et ça évite une migration
 * sur une base de production en service.
 *
 * Un seul mail récapitulatif par admin, pas un par personne — trois demandes en
 * attente ne doivent pas faire trois notifications.
 *
 * `ignorerDelai` saute le délai d'attente sans toucher au plafond de relances :
 * c'est le rattrapage d'un retard déjà constaté, pas une porte pour spammer.
 */
const RELANCES_MAX = 3
const DELAI_RELANCE_JOURS = 3

export async function relancerDemandesEnAttente({ dryRun = false, ignorerDelai = false } = {}) {
  try {
    // Éligibles : moins de RELANCES_MAX relances déjà envoyées, et rien depuis
    // DELAI_RELANCE_JOURS (la demande elle-même compte comme point de départ).
    const enAttente = await sql`
      SELECT
        ...
        u.phone,
        u.approval_token,
        COALESCE(u.approval_requested_at, u.created_at) AS demande_le,
        COUNT(n.id)::int AS relances_envoyees
      FROM users u
      LEFT JOIN notifications_log n
        ON n.notification_type = 'pending_user_reminder'
       AND POSITION('<' || u.email || '>' IN n.content) > 0
      WHERE u.approval_status = 'pending'
        AND u.approval_token IS NOT NULL
      GROUP BY u.id, u.name, u.email, u.phone, u.approval_token, u.approval_requested_at, u.created_at
      HAVING COUNT(n.id) < ${RELANCES_MAX}
         AND (
           ${ignorerDelai}::boolean
           OR COALESCE(
                MAX(n.sent_at),
                u.approval_requested_at,
                u.created_at
              ) < NOW() - (${DELAI_RELANCE_JOURS} * INTERVAL '1 day')
         )
      ORDER BY COALESCE(u.approval_requested_at, u.created_at) ASC
    `

    const demandes = enAttente.rows
    if (demandes.length === 0) {
      return { success: true, relancees: 0, demandes: [] }
    }

    const admins = await getAdminsANotifier()
    if (admins.length === 0) {
      console.warn('Relance demandes d\'accès : aucun admin joignable')
      return { success: false, error: 'Aucun admin', demandes }
    }

    const baseUrl = process.env.NEXTAUTH_URL || 'https://foxfood.fr'
    const jours = d => Math.max(0, Math.floor((Date.now() - new Date(d).getTime()) / 86400000))

    const lignes = demandes.map(d => {
      const attente = jours(d.demande_le)
      const rang = d.relances_envoyees + 1
      const approveUrl = `${baseUrl}/api/admin/approve-user?token=${d.approval_token}&action=approve`
      const rejectUrl = `${baseUrl}/api/admin/approve-user?token=${d.approval_token}&action=reject`
      return `
        <div style="background: #ffffff; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; margin-bottom: 12px;">
          <p style="margin: 0 0 4px 0; font-weight: bold; color: #111827;">${d.name || d.email}</p>
          <p style="margin: 0 0 4px 0; color: #4b5563; font-size: 14px;">${d.email}</p>
          ${d.phone ? `<p style="margin: 0 0 4px 0; color: #6b7280; font-size: 13px;">${d.phone}</p>` : ''}
          <p style="margin: 0 0 12px 0; color: #b45309; font-size: 13px;">
            En attente depuis ${attente} jour${attente > 1 ? 's' : ''}${rang > 1 ? ` · relance ${rang}/${RELANCES_MAX}` : ''}
          </p>
          <a href="${approveUrl}" style="display: inline-block; background: #16a34a; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 14px; margin-right: 8px;">✅ Valider l'accès</a>
          <a href="${rejectUrl}" style="display: inline-block; background: #dc2626; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 14px;">❌ Refuser</a>
        </div>`
    })

    const n = demandes.length
    const subject = `⏳ ${n} demande${n > 1 ? 's' : ''} d'accès attend${n > 1 ? 'ent' : ''} ta réponse`
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #ea580c;">⏳ ${n} demande${n > 1 ? 's' : ''} d'accès en attente</h2>
        <p>Ces personnes ont créé un compte sur FoxFood et attendent ta réponse pour pouvoir commander.</p>
        ${lignes.join('')}
        <p style="color: #6b7280; font-size: 13px; margin-top: 24px;">
          Tu peux aussi tout traiter depuis
          <a href="${baseUrl}/admin/utilisateurs" style="color: #ea580c;">la page Utilisateurs</a>,
          bloc « Demandes d'accès en attente » en haut de l'écran.
        </p>
        <p style="color: #9ca3af; font-size: 12px;">
          Après ${RELANCES_MAX} relances sans réponse, ce rappel s'arrête — les demandes restent visibles sur la page Utilisateurs.
        </p>
      </div>
    `
    ...
    if (dryRun) {
      return {
        ...
        dryRun: true,
        relancees: n,
        destinataires: admins.map(a => a.email),
        demandes: demandes.map(d => ({
          email: d.email,
          nom: d.name,
          attente_jours: jours(d.demande_le),
          relance: d.relances_envoyees + 1
        }))
      }
    }
    ...
    const status = results.every(r => r.success) ? 'sent' : 'failed'

    // Une ligne PAR demande : c'est elle qui sert de compteur au prochain passage.
    for (const d of demandes) {
      await logNotification({
        notification_type: 'pending_user_reminder',
        recipient_email: admins.map(a => a.email).join(', '),
        method: 'email',
        subject,
        content: `Relance ${d.relances_envoyees + 1}/${RELANCES_MAX} — ${d.name || ''} <${d.email}>`,
        status
      })
    }

    return { success: status === 'sent', relancees: n, demandes }
  } catch (error) {
